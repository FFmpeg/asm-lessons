...
많은 예제에서 x86inc.asm 파일을 포함하는 것을 볼 수 있습니다. x86inc.asm는 FFmpeg, x264, dav1d에서 사용하는 가벼운 추상화 레이어로, 어셈블리 프로그래머의 작업을 훨씬 수월하게 만들어 줍니다. 다양한 기능을 제공하지만, 우선 가장 유용한 기능 중 하나는 범용 레지스터(GPR)을 `r0`, `r1`, `r2` 처럼 라벨링해 준다는 점입니다. 덕분에 실제 레지스터 이름을 일일이 외울 필요가 없습니다. 앞서 언급했듯 GPR은 주로 보조의 역할을 하기 때문에 이런 기능은 작업을 훨씬 간단하게 만들어 줍니다.
